...
std::vector<std::vector<double>> do_run(
...
#endif
		trainset_matrix[file_index - 1][0] = directory_index + 1;
...
#endif
			testset_matrix[file_index - 1][0] = directory_index + 1;
	...
	while (trainset_row_index < trainset_matrix.size()) {
		...
		pattern << movement;
#endif
		pattern_matrix[movement - 1][0] = movement;
...
#endif
			pattern_matrix[movement - 1][acc_index + 1] = value;
		}
#if LOGFILE
		pattern << std::endl;
#endif
	...
	for (size_t testset_row_index = 0; testset_row_index < 360; testset_row_index++) {
		...
		if (testset_lines > 0) {
			...
			const uint64_t movement_index = testset_matrix[testset_row_index][0] - 1;
